        ...
        <div className="overflow-hidden w-full px-4 md:px-0">
          <div ref={scrollContainerRef} className="flex flex-col md:flex-row md:flex-nowrap gap-4 md:gap-6 md:overflow-x-auto md:snap-x md:snap-mandatory scroll-smooth pb-4" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {[1, 2, 3, 4, 5, 6].map((item, index) => (
              <div key={index} className={`relative shrink-0 w-full md:w-[calc(25%-18px)] aspect-video md:aspect-4/3 rounded-2xl overflow-hidden group cursor-pointer shadow-md hover:shadow-xl transition-all duration-300 md:snap-start ${!showAllPrograms && index >= 3 ? 'hidden md:block' : 'block'}`}>
                <img src="/candi.png" alt="Program" className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent z-10"></div>
                <div className="absolute bottom-0 left-0 p-5 z-20">
                  <h4 className="text-white font-bold text-lg md:text-xl leading-tight mb-1">Meet the CEOs</h4>
                  <p className="text-gray-300 text-xs md:text-sm">Hands-on workshops</p>
                </div>
              </div>
            ))}
          </div>
